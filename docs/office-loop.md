...
## 오피스가 일을 못 했던 이유와 고친 것 (2026-09-03)

초기화 후 6시간, 오피스 엔진의 5분 마크 누적은 -1.72%로 셋 중 꼴찌였고 제안 확신도는
항상 1.0이었다. 원인은 오피스가 판단 조직이 아니라 **스캐너가 건네준 "20일 모멘텀 1등
5개"의 비중을 정하는 서식 파이프라인**이었다는 것이다. 차트 데스크는 "MA20 위", 뉴스
데스크는 "surges/jumps/rally"를 세어 BULLISH, 퀀트는 μ̂=0.029%/d(t≈0.1, 우위 없음)를
켈리 "상한"으로 적어 놓고 그 상한까지 샀다. 9/9 승인, 수정 0라운드. 고친 것:

| 문제 | 고침 | 코드 |
|---|---|---|
| 바구니 = 스캐너 상위 5 (이미 가장 오른 코인) | 후보 = 메이저 + 보유분 + **60일 위험조정 수익** 상위(신호 엔진의 20일과 다른 지평), 20일 +40% 이상은 과열로 제외(보유 중이면 남김), +25% 이상은 hot | `office/candidates.ts` (순수, 테스트) |
| "다섯 다 아니다"를 낼 수 없음 | 스코프 문구에 "포지션 없음은 정식 답"을 명시. 위원장 JSON의 전부 0이 그대로 현금 결정 | `loop.ts buildScope` |
| 확신도 = 통과 단계 비율 (항상 1.0) | 확신도 = 퀀트 데스크가 잰 우위 **t = μ̂/σ̂·√n** → 0.3 + 0.3t, [0.1, 0.9], 포지션 가중 평균. 검토 라운드가 소진된 채 반대가 남으면 ×0.7. 현금 결정은 0.5. 위원장 JSON `confidence`로 나가고 loop이 제안 확신도로 쓴다 | `office/conviction.ts` (순수, 테스트), `decision.ts`, `loop.ts` |
| 뉴스 데스크가 펌핑 후행 보도를 강세 근거로 | hot/과열 코인에서 BULLISH 헤드라인의 절반 이상이 "surge/jump/rally/soar/breakout"류면 뉴스 기여 0 ("이미 일어난 급등의 보도") | `conviction.ts pumpHeadlineShare` + 모델 뷰 |
| 과열 코인을 그대로 삼 | 모델 뷰: 20일 ≥ +40% → 점수 −1(나가기/회피), ≥ +25% → 비중 절반 | `local-office.ts` |
| 워커 도구가 호출당 5코인 | 후보가 8개까지 늘어 5개씩 나눠 부르고 섹션을 이어 붙임 (순차, Upbit 속도 제한). 리스크 검토는 비중 상위 5개 | `local-office.ts toolOverCoins` |

여전히 안 고친 것: 데스크들이 같은 Upbit 일봉을 읽는다는 사실 자체. 후보 선정 지평을
60일로 바꿔 신호 엔진과 갈라놓았지만, 차트·퀀트 데스크의 계산은 그대로다. 오피스와
신호의 "2명 동의"가 얼마나 독립적인지는 귀속의 두 엔진 누적이 갈리는지로 판단한다.
...
매매 쪽은 데스크 거래 모드를 따른다 — 기본 paper, owner가 설정 페이지에서
REAL로 켜야만 `rotateTo`가 실계좌로 나간다. 둘 다 명시적 사람 손 없이는 실돈에
닿지 않는다.
