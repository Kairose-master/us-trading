...
| 창구 | 하는 것 | 하지 않는 것 |
|---|---|---|
| **TRUST404 사이트** (trust404-prover.vercel.app) | "Museum" 탭: 전시물 12개 + 각 PoC + SCAM 패널(mint·수수료 지갑 잔고·바운티 풀). 홍보의 본진 | 가격 차트 전면 배치 |
...
1. 트위터·텔레그램 채널 개설(고지문 고정) → 발행 카드에 링크 입력 → `LAUNCH`. dev buy 는 소각.
2. TRUST404 사이트에 Museum 탭 + SCAM 패널(mint·수수료 지갑·바운티 풀). `/api/prove` 에 지갑 서명 티어(①).
3. 대시보드에 자기 감사 패널(⑥) — 코드는 이미 있다(커뮤니티 데스크), 우리 mint 만 읽기 허용하면 된다.
4. 첫 바운티 공고(③) — 수수료 풀이 0이면 발행자가 시드 0.5 SOL.
5. 전시물 스레드 12개를 주 2개씩(②의 재료).
