...
## 알려진 한계 — 시험지가 한 장이다 (2026-09-03)

시험 구간은 항상 **가장 최근 60일**(`buildFeatures(series, EXAM_DAYS)` → `trainEnd = dates.length − 60`)이다.
세대가 6시간마다 도니 시험지는 세대마다 겨우 6시간씩 밀린다 — 사실상 같은 60일을 다시 본다.
유전 알고리즘이 운과 적합성을 가르는 힘은 **세대마다 다른 시험지**에서 나온다: 매번 다른
구간을 통과한 개체만 남는 것이 선택이고, 같은 구간을 반복해서 통과하는 것은 그 구간에 대한
과적합과 구분되지 않는다. 지금 구조에서 "3세대 연속 하위 20% 도태"는 같은 시험의 재채점이다.
고치는 방법은 작다: 세대마다 훈련 구간 밖에서 60일 창을 무작위(또는 롤링)로 뽑아 `evaluate`의
`window` 인자로 넘기고, 적합도 이력을 창이 다른 시험의 이력으로 읽는 것. 이 문서에 적은 것은
그것이 아직 안 된 상태라는 사실이다.
