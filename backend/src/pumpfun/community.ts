...
export class CommunityDesk {
  ...
  noteLaunch(creator: string, ts = Date.now()) {
    if (!creator) return;
  ...
  private async facts(mint: string, timeoutMs: number): Promise<CommunityFacts> {
    ...
    const [tg, top] = await Promise.all([f.telegram ? this.telegramMembers(f.telegram, Math.min(timeoutMs, 4_000)) : Promise.resolve(null), config.PUMPFUN_RPC_URL ? this.top10(mint).catch(() => null) : Promise.resolve(null)]);
