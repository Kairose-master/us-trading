...
export interface PumpPolicy { maxPositionSol: number; riskPct: number; grossMaxPct: number; cashFloorPct: number; maxLots: number; minLeaderSol: number; maxHoldMin: number; timeStopExemptPct: number; stopLossPct: number; trailingPct: number; trailingActivatePct: number; ladder: Array<{ atPct: number; fraction: number }>; followMax: number; eta: number; dropAtPct: number; dropAfterCloses: number; dailyStopPct: number; discoveryWindowMin: number; discoveryMaxMints: number; rescoreMin: number; meteredBudgetMsgsPerDay: number; subscribeHeldTokens: number; maxLeaderFlips10m: number; minLeaderHoldMin: number; reentryCooldownMin: number; directCopy: number; flowMaxMints: number; convictionMode: number; convictionPct: number; convictionMinScore: number; maxConvictionLots: number; floodMintPerMin: number; floodCooldownMin: number; crashPct: number; crashWindowSec: number }
export interface PumpEnsembleCandidate { mint: string; symbol: string | null; score: number; action: "enter" | "hold" | "exit" | "none"; why: string; blocked: boolean; sizeMult: number; votes: Array<{ engine: "flow" | "momentum" | "copy" | "community"; score: number; abstain: boolean; why: string[] }>; streamed: boolean; held: number }
export interface PumpEnsemble { weights: { flow: number; momentum: number; copy: number; community: number }; policy: { enterScore: number; exitScore: number; basePct: number; flowReversalSol: number; fadeFromPeakPct: number; minFlowTrades: number }; directCopy: number; flowMaxMints: number; conviction: { mode: number; pct: number; minScore: number; maxLots: number; open: number }; elastic: { msgsPerMin: number; pacePerMin: number; activeFlowMax: number; floodBlocked: number; topMintRates: Array<{ mint: string; perMin: number }> }; screen: { candidates: number; lastPollAt: string | null; polls: number; coinsSeen: number }; stats: { evaluations: number; entries: number; exits: number }; blocks?: Array<{ ts: string; mint: string; why: string }>; candidates: PumpEnsembleCandidate[] }
