import type { CryptoCandle } from "./upbit"

/**
 * 알파 백테스트 엔진 — backend/src/crypto/backtest.ts와 동일 로직·규약.
 * 시그널은 t 종가까지의 정보로 t+1 목표 포지션(0|1)을 정한다 — 룩어헤드 없음,
 * 현물이라 공매도 없음. 한쪽을 고치면 다른 쪽도 맞출 것.
 */
...
export interface AlphaSignal {
  ...
  description: string
  code: string
  position: (candles: CryptoCandle[], i: number) => 0 | 1
}
...
export function runBacktest(candles: CryptoCandle[], signal: AlphaSignal, market: string): BacktestResult {
  ...
  const sd = Math.sqrt(dailyRets.reduce((a, r) => a + (r - mean) ** 2, 0) / Math.max(1, dailyRets.length))
