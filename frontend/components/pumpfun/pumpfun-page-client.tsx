    ...
    <div className="flex flex-col gap-4">
              ...
              <table className="w-full font-mono text-[11px]">
                ...
                <tbody className="divide-y divide-border/50">
                  {data.live.lots.map((l) => (
                    <tr key={l.id} onClick={() => window.open(`https://pump.fun/coin/${l.mint}`, "_blank", "noopener,noreferrer")} className="cursor-pointer hover:bg-muted/40" title={`pump.fun 에서 ${l.mint} 열기`}><td className="px-3 py-1"><a href={`https://pump.fun/coin/${l.mint}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="underline decoration-dotted underline-offset-2">{short(l.mint)}</a></td><td className="px-3 py-1 text-muted-foreground" title={l.via}>{short(l.via)}</td><td className="px-3 py-1">{l.pool}</td><td className="px-3 py-1">{l.costSol.toFixed(4)}</td><td className="px-3 py-1">{l.markSol.toFixed(4)} <span className="text-muted-foreground">{ago(l.markAt)}</span></td><td className={cn("px-3 py-1 font-bold", pnlClass(l.pnlPct))}>{signed(l.pnlPct)}</td><td className="px-3 py-1">{l.holdMin.toFixed(0)}분</td><td className="px-3 py-1"><CommunityCell c={l.community} /></td></tr>
                  ))}
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-1.5 border-b border-border px-4 py-2.5">
          ...
          {(data.ensemble.blocks ?? []).length > 0 && (
            <div className="w-full rounded-md border border-border bg-muted/30 px-2 py-1.5 font-mono text-[10px]">
              <div className="mb-0.5 font-semibold text-foreground">왜 안 샀나 (최근)</div>
              {(data.ensemble.blocks ?? []).slice(0, 8).map((b, i) => (<div key={i} className="text-muted-foreground">{ago(b.ts)} · {b.mint.slice(0, 6)} · {b.why}</div>))}
            </div>
          )}
        </div>
        <div className="overflow-x-auto">
