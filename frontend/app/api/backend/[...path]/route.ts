...
const ALLOW_GET: RegExp[] = [
  ...
  /^control$/,
  /^pumpfun(\/(wallets|orders|equity|events|quote|mode|launch))?$/,
]
const ALLOW_WRITE: Array<{ method: string; re: RegExp }> = [
  ...
  { method: "POST", re: /^pumpfun\/(wallets|rescore|pause|resume|policy|mode|live\/(policy|flatten|reconcile)|launch(\/preview)?)$/ },
]
