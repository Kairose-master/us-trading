...
const ALLOW_GET: RegExp[] = [
  ...
  /^quotes\/[^/]+(\/chart)?$/,
  /^risk\/limits$/,
  /^crypto\/(status|quotes|signals|paper\/equity|universe|candles(\/[A-Za-z0-9-]+)?|scanner(\/backtest|\/spa)?|contracts|contract\/[A-Za-z0-9-]+|pipeline(\/nodes\/[^/]+|\/logs)?|sentiment(\/feed)?)$/,
